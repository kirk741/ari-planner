...
const HomePage = () => {
  ...
  return (
    <div className="home-page">
      <h1 className="page-title">Ежедневник</h1>
          ...
          <div className="next-event-card" onClick={() => navigate(`/event/${nextEvent.id}`)}>
            <div className="next-event-date">
              ...
              <span className="month">
                {new Date(nextEvent.date).toLocaleString('ru', { month: 'short' })}
              </span>
            </div>
            <div className="next-event-info">
              <h3>{nextEvent.title}</h3>
              <p className="next-event-time">
                {new Date(`${nextEvent.date}T${nextEvent.startTime}`).toLocaleString('ru', {
      ...
      <div className="view-all-button" onClick={() => navigate('/events')}>
        Все записи <span className="arrow">→</span>
      </div>
